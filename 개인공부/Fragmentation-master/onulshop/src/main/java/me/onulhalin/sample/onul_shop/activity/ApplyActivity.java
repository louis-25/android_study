...
public class ApplyActivity  extends OHActivitiy implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new CustomAdapter( mArrayList,getParent());
            ...
            public void onResponse(Call<CallbackProduct> call, Response<CallbackProduct> response) {
                ...
                if (resp != null) {
                    ...
                    for (int i =0; i<resp.products.size(); i++) {
                        ...
                        Dictionary dict = new Dictionary(resp.products.get(i).id+"", "" + resp.products.get(i).name, "" + resp.products.get(i).stock,resp.products.get(i).image);//, resp.products.get(i).draft);
...
//                break;
    ...
    private void submitData(int id,int stock) {
        API api = RestAdapter.createAPI();
        Call<CallbackProduct> callbackCall = api.setUpdateStock(id,stock);
...
// MainActivity 에서 요청할 때 보낸 요청 코드 (3000)
                case 3000:
                    ...
                    submitData(Integer.valueOf(data.getStringExtra("result")),Integer.valueOf(data.getStringExtra("stock")));
