...
public class LoginActivity extends OHActivitiy implements View.OnClickListener {
            ...
            public void onResponse(Call<CallbackDevice> call, Response<CallbackDevice> response) {
                ...
                //    Log.d("kimtest","@@" +resp.user.name);
                   ...
                   // editor.putBoolean("Auto_Login_enabled", true);
                    ...
                    editor.commit();
                    //출퇴근 api
                        shopUserCommute(adminid.getText().toString());

                }
                else if (resp != null && resp.status.equals("failed")){
                    Toast.makeText(LoginActivity.this, "아이디와 비밀번호 확인해주세요", Toast.LENGTH_SHORT).show();
                }
                else if (resp != null && resp.status.equals("notaccept")){
                    Toast.makeText(LoginActivity.this, "관리자의 승인을 받지 않았습니다.", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void shopUserCommute(String userid) {
