...
public class ProductAddActivity extends OHActivitiy implements View.OnClickListener {
    ...
    EditText price;
    EditText stock;
    public List<Category> categorie = new ArrayList<>();
    String category ;
    ...
    Intent intent;
    EditText status;
    TextView pickstart,pickend;
    String ptStart , ptEnd;
    CheckBox delivery;
    CheckBox timesale,endsale,exhibitions,ready,ing,end;
    String type,ingstatus;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pickstart =(TextView)findViewById(R.id.pickstart);
        pickend= (TextView)findViewById(R.id.pickend);
        pickend.setOnClickListener(this);
        pickstart.setOnClickListener(this);
        delivery = (CheckBox)findViewById(R.id.delivery);
        timesale = (CheckBox)findViewById(R.id.timesale);
        endsale = (CheckBox)findViewById(R.id.endsale);
        exhibitions = (CheckBox)findViewById(R.id.exhibitions);


        ready =  (CheckBox)findViewById(R.id.ready);
        ing =  (CheckBox)findViewById(R.id.ing);
        end =  (CheckBox)findViewById(R.id.end);
...
//                                messageText.setText("uploading started.....");
    ...
    public void init(){
        ...
        ing.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    ingstatus = "메뉴준비중";
                    ing.setTextColor(Color.RED);
                    end.setTextColor(Color.BLACK);
                    end.setChecked(false);
                    ready.setTextColor(Color.BLACK);
        ...
        ready.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    ingstatus = "판매중";
                    ready.setTextColor(Color.RED);
                    end.setTextColor(Color.BLACK);
                    end.setChecked(false);
                    ing.setTextColor(Color.BLACK);
                    ing.setChecked(false);
                }
            }
        });
        ...
        timesale.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    type = "타임세일";
                    timesale.setTextColor(Color.RED);
                    endsale.setTextColor(Color.BLACK);
                    endsale.setChecked(false);
                    exhibitions.setTextColor(Color.BLACK);
                    exhibitions.setChecked(false);
                }
            }
        });

        endsale.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    type = "마감세일";
                    endsale.setTextColor(Color.RED);
                    timesale.setTextColor(Color.BLACK);
                    timesale.setChecked(false);
                    exhibitions.setTextColor(Color.BLACK);
                    exhibitions.setChecked(false);
                }
            }
        ...
        exhibitions.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    type = "기획전";
                    exhibitions.setTextColor(Color.RED);
                    timesale.setTextColor(Color.BLACK);
                    timesale.setChecked(false);
                    endsale.setTextColor(Color.BLACK);
                    endsale.setChecked(false);
    ...
    public int uploadFile(String sourceFileUri) {
        ...
        if (!sourceFile.isFile()) {
            ...
            try {
                ...
                bufferSize = Math.max(maxBufferSize, maxBufferSize);
                ...
                while (bytesRead > 0) {
                    ...
                    bytesAvailable = fileInputStream.available();
                    bufferSize = Math.min(maxBufferSize, maxBufferSize);
                    bytesRead = fileInputStream.read(buffer, 0, bufferSize);
                }
        ...
        if(requestCode==0){
            if(grantResults[0]==0){
               // Toast.makeText(this,"카메라 권한이 승인됨",Toast.LENGTH_SHORT).show();
                ...
                //권한 거절된 경우
                Toast.makeText(this,"권한이 거절 되었습니다.상품등록을 이용하려면 권한을 승인하여야 합니다.",Toast.LENGTH_SHORT).show();
                finish();
            }
        }
    }
...
//        user.price = adminpw.getText().toString();
        ...
        user.draft = 0;
        user.status = ingstatus;
        user.userid= setting.getString("ID", "");
        user.ptstart = ptStart;
        user.ptend = ptEnd;
        user.type = type;
...
//        user.price = adminpw.getText().toString();
    ...
    private void requestListCategory() {
        ...
        callbackCall.enqueue(new Callback<CallbackCategory>() {
            @Override
            public void onResponse(Call<CallbackCategory> call, Response<CallbackCategory> response) {
                final CallbackCategory resp = response.body();
                if (resp != null && resp.status.equals("success")) {

                    categorie = resp.categories;
                    for (int i =0; i< categorie.size(); i++){
                        Log.d("kimtest","##"+categorie.get(i).name);
                        status.append(categorie.get(i).name);
                    }
                    ...
                    for(int i=0; i< categorie.size() ; i++){
                        ...
                        rb[i].setText(" " + categorie.get(i).name);
                        rb[i].setId(resp.categories.get(i).id);
                        rg.addView(rb[i]);
                    }
                    test.addView(rg);//you add the whole RadioGroup to the layout
...
//                    onFailRequest();
    ...
    private TimePickerDialog.OnTimeSetListener listener = new TimePickerDialog.OnTimeSetListener() {
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
            ...
            String pickstminute = String.valueOf(minute);
// 설정버튼 눌렀을 때
            if (minute==0){
                pickstminute = "00";
            }
...
// 설정버튼 눌렀을 때
            pickstart.setText(hourOfDay + "시 " + pickstminute + "분");
            ptStart = ""+hourOfDay+":"+pickstminute;
            //  Toast.makeText(getApplicationContext(), hourOfDay + "시 " + pickstminute + "분", Toast.LENGTH_SHORT).show();
    ...
    private TimePickerDialog.OnTimeSetListener listener2 = new TimePickerDialog.OnTimeSetListener() {
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
            ...
            String pickendminute = String.valueOf(minute);
// 설정버튼 눌렀을 때
            if (minute==0){
                pickendminute = "00";
            }
            pickend.setText(hourOfDay + "시 " + pickendminute + "분");
            ptEnd = ""+hourOfDay+":"+pickendminute;
            //    Toast.makeText(getApplicationContext(), hourOfDay + "시 " + pickendminute + "분", Toast.LENGTH_SHORT).show();
    ...
    public void onClick(View view) {
        switch (view.getId()){
            ...
            case R.id.pickend:
                ...
                TimePickerDialog pickend = new TimePickerDialog(this, listener2, 15, 24, false);
                pickend.show();
                break;
        }
    }
